...
float eTempLevel = 5.00; //----------------------added
float humLevel = 5.00; //-------------------------added
float salLevel;
...
void loop() {
 ...
 // Read time and Setpoints and update them -------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if ( abs(timeDiff) >= readingsInterval || LastReadingDayCount > dayCount) { //added absolute so it does not get stuck not taking a reading
...
void salDosing( float salLevel) {
  ...
  if (salLevel > salSetPoint) {
    ...
    delay(120000); // change delay for amount dosed
...
void waterTempCorrect( float waterTempLevel) {
  ...
  if (waterTempLevel > wTempSetPoint) {

    Serial.print("ewtp");
    Serial.print("Water Temp above Setpoint, Temp Reading:" );
    Serial.print(waterTempLevel);
    Serial.print(", Temp Reading:" );
    Serial.println(wTempSetPoint);
  }
}
