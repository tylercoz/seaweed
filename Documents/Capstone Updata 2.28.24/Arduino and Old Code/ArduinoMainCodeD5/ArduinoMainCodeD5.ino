...
//Setpoints
float humSetPoint = 2.00;
float phSetPoint = 2.00;
float salSetPoint = 2.00;
float eTempSetPoint = 2.00;
float wTempSetPoint = 2.00;
float boxHumSet = 30;
float boxTempSet = 90;
...
void setup() {
  ...
  pinMode(enableDos1,OUTPUT);
  pinMode(enableDos2,OUTPUT);
  pinMode(Dos1R,OUTPUT);
  pinMode(Dos1L,OUTPUT);
  pinMode(Dos2R,OUTPUT);
  pinMode(Dos2L,OUTPUT);
...
void loop() {
 ...
 // Read time and Setpoints and update them -------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if ( (hour*60 + min) - (LastReadingHour*60 + LastReadingMin) >= readingsInterval || LastReadingDayCount >= dayCount) {
    ...
    if ( (hour*60 + min) - (LastSalHour*60 + LastSalMin) >= (readingsInterval*2) || LastSalDayCount >= dayCount) {
    ...
    if ( (hour*60 + min) - (LastPhHour*60 + LastPhMin) >= (readingsInterval*2) || LastPhDayCount >= dayCount) {
    ...
    if ( (hour*60 + min) - (LastFanHour*60 + LastFanMin) >= (readingsInterval*2) || LastFanDayCount >= dayCount) {
...
void phDosing( float phLevel){
  ...
  if (phLevel > 0.1*phSetPoint + phSetPoint) {
    ...
    digitalWrite(Dos1R, LOW);
    digitalWrite(Dos1L, HIGH);
    analogWrite(enableDos1, 177);

    delay(4000); // change delay for amount dosed

    //Stop Dosing
    analogWrite(enableDos1, 0);
  }
}
...
void salDosing( float salLevel) {
  ...
  if (salLevel > 0.2*salSetPoint + salSetPoint) {
    ...
    //H-Bridge peristaltic dosing
    digitalWrite(Dos2R, LOW);
    digitalWrite(Dos2L, HIGH);
    analogWrite(enableDos2, 177);

    delay(4000); // change delay for amount dosed

    //Stop Dosing
    analogWrite(enableDos2, 0);
  }
}
