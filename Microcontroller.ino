...
Ezo_board ph = Ezo_board(99, "PH"); //create a PH circuit object, who's
address is 99 and name is "PH"
Ezo_board rtd = Ezo_board(102, "TEMP"); //create an RTD Temperature circuit
object who's address is 102 and name is "TEMP"
...
char cond_data[32]; //we make a 32-byte character array to hold incoming
data from the EC sensor.
char *cond_str; //char pointer used in string parsing.
char *TDS; //char pointer used in string parsing.
char *SAL; //char pointer used in string parsing (the
sensor outputs some text that we don't need).
...
float TDS_float; //float var used to hold the float value of the
total dissolved solids.
float SAL_float; //float var used to hold the float value of the
salinity.
float SG_float; //float var used to hold the float value of the
specific gravity.
float HUM_float;
float TE_float;
float phLevel;
...
Serial.begin(9600, SERIAL_8N1); // Set the hardware
serial port.
...
// Turn on everything (water pump, lights, filter?) -- Only need circulation on
will check if rest needs to be on based on time
...
if ( message.charAt(0) == 's') { // Check if message is not empty and starts
with 's'
...
if ( (hour*60 + min) - (LastSalHour*60 + LastSalMin) >= (readingsInterval) ||
...
if ( (hour*60 + min) - (LastPhHour*60 + LastPhMin) >= (readingsInterval) ||
...
if ((hour > nut_dos_hour) || (hour == nut_dos_hour && nut_dos_min >=
nut_dos_min)) {
nutDosing();
...
if ( (hour*60 + min) - (LastFanHour*60 + LastFanMin) >= (readingsInterval) ||
...
cond_str = strtok(cond_data, ","); //let's parse the string at each
comma.
TDS = strtok(NULL, ","); //let's parse the string at each
comma.
SAL = strtok(NULL, ","); //let's parse the string at each
comma
SG = strtok(NULL, ",");
...
// PMP_UP.send_cmd_with_num("d,",15); // need to figure out how to
volumetrically dispense
...
// hvacOn(HVACPin); //---- Put in while loop if we are indeed in temp
controlled room
// delay(5000); //--------------------------------------------------------------------------------------make more scientifc (poll the temp again after thedelay and check that reading is good)
// hvacOff(HVACPin);
// }
}
...
//Read fload Swtich method with flaot pin and pin to shut off with shutoff call
if triggered
...
if (firstChar == 's'|| firstChar == 't') { // Check if the first character is
's'
